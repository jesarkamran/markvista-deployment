        ...
        switch (selectedTab) {
          case "positions":
            ...
            response = await fetch(
              "http://localhost:4000/api/order/active-positions",
          ...
          case "pendingOrders":
            response = await fetch(
              "http://127.0.0.198:4000/api/order/order-list",
            );
            // Replace with your endpoint
          ...
          case "tradeHistory":
            ...
            var tradeHistory = await response.json();
  ...
  const confirmCancel = async () => {
    try {
      ...
      const response = await fetch(
        "http://localhost:4000/api/order/cancel-order",
        {
        ...
        <div className="positions-section">
          ...
          {positionsData.map((position, index) => (
            <div key={index} className="data-row">
              ...
              <span>
                {position.takeProfit || "N/A"} / {position.stopLoss || "N/A"}
              </span>
            </div>
          ))}
        </div>
        ...
        <div className="pending-orders-section">
          ...
          {pendingOrdersData.map((order, index) => (
            <div key={index} className="data-row">
              ...
              <span>{order.orderStatus}</span>
              <span>
                {new Date(parseInt(order.createdTime)).toLocaleString()}
              </span>
              <span className="close-by">
                <button
        ...
        <div className="trade-history-section">
          ...
          {tradeHistoryData.map((history, index) => (
            <div key={index} className="data-row1">
              ...
              <span>{history.side === "buy" ? "Short" : "Long"}</span>{" "}
              {/* Change buy/sell to Long/Short */}
              <span>{parseFloat(history.closedPnl).toFixed(2)}</span>{" "}
              {/* Ensure it's a number and fix to 2 decimal places */}
              <span>{parseFloat(history.avgEntryPrice).toFixed(2)}</span>{" "}
              {/* Ensure it's a number and fix to 2 decimal places */}
              <span>{parseFloat(history.qty).toFixed(2)}</span>{" "}
              {/* Ensure it's a number and fix to 2 decimal places */}
              <span>{parseFloat(history.orderPrice).toFixed(2)}</span>{" "}
              {/* Ensure it's a number and fix to 2 decimal places */}
              <span>{parseFloat(history.avgExitPrice).toFixed(2)}</span>{" "}
              {/* Ensure it's a number and fix to 2 decimal places */}
              <span>{history.execType}</span>
              <span>{history.closedSize}</span>
              <span>
                {new Date(parseInt(history.updatedTime)).toLocaleString()}
              </span>
            </div>
          ))}
        </div>
