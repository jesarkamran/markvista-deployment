...
const Order = ({ id, title, onDelete, symbol, setSymbol }) => {
  ...
  const onToggle = async (id) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:4000/api/riskprofiles/${id}/activate`,
        {
    ...
    const fetchData = async () => {
      try {
        ...
        const balanceResponse = await fetch(
          "http://localhost:4000/api/order/showusdtbalance",
        );
  ...
  useEffect(() => {
    const fetchRiskProfiles = async () => {
      try {
        ...
        const response = await fetch(
          "http://localhost:4000/api/riskprofiles/getactive",
        );
        if (!response.ok) {
  ...
  const handleOpenLong = async () => {
    if (
      parseFloat(stopLoss) >= parseFloat(orderPrice) ||
      parseFloat(takeProfit) <= parseFloat(orderPrice)
    ) {
      alert(
        "For Open Long: Stop Loss must be lower and Take Profit must be higher than Order Price",
      );
    } else {
  ...
  const handleOpenShort = async () => {
    if (
      ...
      alert(
        "For Open Short: Stop Loss must be higher and Take Profit must be lower than Order Price",
      );
    } else {
          ...
          <div className="position-section">
            <div>
              <div>
                ...
                <div>Risk Profile is Active</div>
              </div>
            </div>
          </div>
          ...
          <div className="action-buttons">
            {positionType === "open" ? (
              <div className="button-group">
                <button
                  className="form-button btn-buy"
                  onClick={handleOpenLong}
                >
                  Open Long
                </button>
                <button
                  className="form-button btn-sell"
