...
const Accgrowth = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const transformedData = transformPnLData(
          pnlResult.trades,
          currentBalance,
        );
  ...
  const transformPnLData = (trades, currentBalance) => {
    ...
    const transformedData = Object.values(groupedData)
      ...
      .map((entry) => {
        ...
        return {
          date: entry.date,
          AccountBalance: cumulativeBalance.toFixed(2),
        };
      });
    ...
    transformedData.unshift({
      date: "Initial",
      AccountBalance: initialBalance.toFixed(2),
    });
  ...
  return (
    <div>
      ...
      <Achart data={data} />{" "}
      {/* Passing the chart data to the chart component */}
    </div>
  );
};
