...
import AdjustedRisk from "../../model/b-risk-module/riskprofilemodal.js"; // Import the AdjustedRisk model
import DailyLoss from "../../model/b-risk-module/riskprofilemodal.js"; // Adjust path according to where you defined the model
...
const calculateAdjustedRisk = (
  ...
  if (lastTradeResult === "Win") {
    ...
    console.log(
      `Risk increased by ${increaseOnWin}%. New risk: ${adjustedRisk}%`
    );
  } else if (lastTradeResult === "Loss") {
    adjustedRisk -= (decreaseOnLoss / 100) * adjustedRisk;
    console.log(
      `Risk decreased by ${decreaseOnLoss}%. New risk: ${adjustedRisk}%`
    );
  }

  return Math.max(
    riskProfile.minRisk || 0,
    Math.min(adjustedRisk, riskProfile.maxRisk || 100)
  );
};
...
const simplePlaceOrder = async (data) => {
  try {
    const orderLinkId = randomBytes(16).toString("hex");
...
const placeOrderWithRiskProfile = async (data) => {
  try {
    ...
    try {
      ...
      throw new Error(
        "Failed to fetch PnL data. Please check the network or try again later."
      );
    }
    ...
    if (SLallowedPerDay <= 0) {
      console.warn(
        "SL ALLOWED PER DAY is already hit, come tomorrow to trade."
      );
      return;
    }
    ...
    const activeOrders = orderListResponse?.result?.list?.filter(
      (order) => !order.stopOrderType
    ); // Only include non-conditional orders

    console.log(`Active Orders (Excluding OCO/SLTP): ${activeOrders.length}`);

    // Fetch open positions
    const activePositionsResponse = await getPositionInfo(
      "category=linear&settleCoin=USDT"
    );
    ...
    if (!takeProfit || !stopLoss) {
      throwError(
        "Both takeProfit and stopLoss must be provided to place an order."
      );
    }
    ...
    if (riskRewardRatio < minRiskRewardRatio) {
      throwError(
        `The calculated risk-to-reward ratio (${riskRewardRatio.toFixed(
          2
        )}) is less than the minimum required ratio (${minRiskRewardRatio}). Cannot place the order.`
      );
    }

    console.log(
      `Risk-to-Reward Ratio is acceptable: ${riskRewardRatio.toFixed(
        2
      )} >= ${minRiskRewardRatio}`
    );
    ...
    if (!initialRisk || initialRisk <= 0)
      throwError("Invalid initial risk in active risk profile.");

    if (activeOrders?.length > 0) {
      console.warn(
        "Active market/limit orders found. Cannot place a new order."
      );
      return;
    }
    ...
    const allpnlResponse = await http_request(
      pnlendpoint,
      "GET",
      "category=linear",
      "Get Closed PnL"
    ...
    if (consecutiveWins >= resetValue || consecutiveLosses >= resetValue) {
      console.log(
        "Consecutive wins/losses threshold reached. Resetting to initial risk."
      );
    ...
    if (isFirstTrade) {
      ...
      console.warn(
        "No closed trades available or it's the first trade. Using initial risk."
      );
    }
    ...
    if (!tickerInfo)
      throwError(`Ticker information not found for symbol ${symbol}`);
    ...
    if (riskPerUnit <= 0) {
      throwError(
        "Stop loss price must be less than entry price for long trades, or greater for short trades."
      );
    }
  ...
  try {
    ...
    for (const field of numericFields) {
      if (isNaN(parseFloat(data[field]))) {
        return res
          .status(400)
          .json({ error: `Invalid numeric value for field: ${field}` });
      }
    }
    ...
    if (!riskProfile) {
      ...
      return res
        .status(200)
        .json({ message: "Order placed without risk profile." });
    } else {
      ...
      return res
        .status(200)
        .json({ message: "Order placed with risk profile." });
    }
  } catch (error) {
    console.error("Error in placeOrder function:", error.message);
    return res
      .status(500)
      .json({ error: `Failed to place order: ${error.message}` });
  }
};

const getUsdtBalance = (accountBalanceResponse) => {
  // Find the coin object for USDT in the response
  const usdtCoin = accountBalanceResponse?.result?.list?.[0]?.coin?.find(
    (coin) => coin.coin === "USDT"
  );
...
const cancelOrder = async (req, res) => {
  ...
  try {
    ...
    if (response.retMsg === "OK") {
      ...
      throw new Error(
        "Failed to cancel the order or invalid response from API."
      );
    }
...
const setLeverage = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Leverage updated successfully", response });
    return response;
  } catch (error) {
    console.error("Error setting leverage:", error.message);
    res
      .status(500)
      .json({ error: "Failed to update leverage", details: error.message });
  }
};
...
const switchMarginMode = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: "Margin mode updated successfully", response });
  } catch (error) {
...
function analyzeCoinPerformance(trades) {
  ...
  const bestCoins = [...coinPnLArray]
    .sort((a, b) => b.totalPnL - a.totalPnL)
    .slice(0, 5);
  const worstCoins = [...coinPnLArray]
